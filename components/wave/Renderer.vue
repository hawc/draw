...
<script lang="ts">
  ...
  export default Vue.extend({
    ...
    methods: {
      ...
      getVisibleScreenSize(
        camera: THREE.PerspectiveCamera,
        cameraDistance: THREE.cameraDistance,
      ): ScreenSize {
      ...
      initThree(): void {
        ...
        camera.position.x = -(
          this.getVisibleScreenSize(camera, CAMERA_DISTANCE).x / 2
        );
        ...
        const updateContent = () => {
          ...
          newCube.position.set(
            lastCube.position.x +
              (this.settings.lineWidth + this.settings.lineDistance),
            0,
            0,
          );
          scene.add(newCube);
