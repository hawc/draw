<template>
  <div class="renderer">
    <canvas id="canvasDump" ref="canvasDump" class="skyCanvas"></canvas>
    <main></main>
  </div>
</template>
...
<script>
  ...
  const modelTypes = ['box', 'sphere', 'torus'];
  ...
  export default Vue.extend({
    ...
    computed: {
      ...
      pixelDensity() {
        return (
          (1.7 - this.settings.pixelation) *
          ((0.5 / window.devicePixelRatio) * this.sizeRatio)
        );
      },
      maxDimension() {
    ...
    methods: {
        ...
        gifshot.createGIF(
          ...
          (object) => {
            if (!object.error) {
              ...
              this.SET_DL_NAME(
                `planet-${textures[this.settings.globeTexture]}.gif`,
              );
      ...
      initP5() {
        ...
        this.backLayer = (p) => {
          ...
          p.setup = () => {
            loadedBgTexture = p.loadImage(
              require('@/assets/textures/bg.jpg'),
              (img) => {
          ...
          p.draw = () => {
            ...
            const diameter =
              2 + 1.6 * (this.settings.globeDiameter / 170 - 0.5);
            ...
            bgShader.setUniform(
              'color1',
              this.hexToRgb(this.$shadeHexColor(this.color1, 0.7)),
            );
            bgShader.setUniform('color2', this.hexToRgb('#00000f'));
            bgShader.setUniform(
              'color3',
              this.hexToRgb(this.$shadeHexColor(this.color2, 0.7)),
            );
        ...
        this.frontLayer = (p) => {
          ...
          p.setup = () => {
            ...
            textures.forEach((texture) => {
              loadedTextures.push(
                p.loadImage(require(`@/assets/textures/${texture}.jpg`)),
              );
            });
            loadedRingsTexture = p.loadImage(
              require('@/assets/textures/2d2.png'),
            );
            loadedRingsTexture2 = p.loadImage(
              require('@/assets/textures/2d-donut.png'),
            );
            p.frameRate(30);
          ...
          p.draw = () => {
            frame =
              frame +
              0.0075 * this.settings.rotationSpeed * this.stopMultiplicator;
            loopCount = Math.floor(frame / (2 * Math.PI));
            const frameRotation =
              Math.round((frame % (2 * Math.PI)) * 100) / 100;
            ...
            const modelTypeName =
              modelTypes[this.settings.modelType] ?? 'sphere';
            planetGraphic.clear();
            ...
            planetGraphic.texture(
              loadedTextures[this.settings.globeTexture ?? 0],
            );
            ...
            if (colorPresetUsed) {
              frontShader.setUniform(
                'color1',
                this.hexToRgb(this.colorPresets[colorPresetUsed][0]),
              );
              frontShader.setUniform(
                'color2',
                this.hexToRgb(this.colorPresets[colorPresetUsed][1]),
              );
              frontShader.setUniform(
                'color3',
                this.hexToRgb(this.colorPresets[colorPresetUsed][2]),
              );
            } else {
